          ...
          if (userResult[0] && userResult[0].length > 0) {
            ...
            if (Number.isInteger(parseInt(args[0]))) {
                  ...
                  if (result[0] && result[0].length > 0) {
                    if (result[0][0].facetType == "section") {
                      message.channel.send(
                        "Oop! You searched for the ID of a facet section, to view this section simply search for the ID of the actual facet instead (Hint: section IDs are hidden anyway)"
                      );
                    } else {
                      if (
                        ...
                        firstImage = result[0][0].facetMedia
                          .split(", ")[0]
                          .split(": ")[0];
                      } else {
                        firstImage =
                      ...
                      let linksText =
                        "No links provided. Consider adding one with %wblink [first ID] [second ID] [description of relationship]";
                      if (
                        ...
                        for (var i = 0; i < links.length; i++) {
                          ...
                          linkMap[parseInt(links[i].split(": ")[0])] = links[
                            i
                          ].split(": ")[1];
                        }
                        linkIDsList = linkIDsList.substring(
                          0,
                          linkIDsList.length - 2
                        );
                        console.log(linkIDsList);
                        db.query(
                          `SELECT * FROM guild_${message.guild.id} WHERE facetId IN (${linkIDsList})`
                        )
                          .then((result2) => {
                            console.log("TEST");
                            ...
                            viewEmbed = new Discord.MessageEmbed()
                              ...
                              .addFields(
                                {
                                  name: "Author",
                                  value: this.client.users.cache.get(
                                    result[0][0].facetAuthor
                                  ).tag,
                                },
                                { name: "Type", value: result[0][0].facetType },
                                {
                                  name: "Tag(s)",
                                  value: result[0][0].facetTags,
                                },
                                {
                                  name: "Description",
                                  value: result[0][0].facetDesc,
                                },
                                { name: "Related Links", value: linksText }
                              )
                              .setImage(firstImage)
                              .setFooter(
                                "Consider using imgur to host media",
                                "https://cdn.discordapp.com/app-icons/592736284816965663/8223af4828cc8d2db1c5fffcca3acfa0.png?size=512"
                              );

                            db.query(
                              `SELECT * FROM guild_${message.guild.id} WHERE facetTags = '${result[0][0].facetId}' AND facetType = 'section'`
                            )
                              .then((sectionResult) => {
                                if (
                                  sectionResult[0] &&
                                  sectionResult[0].length > 0
                                ) {
                                  for (
                                    var i = 0;
                                    i < sectionResult[0].length;
                                    i++
                                  ) {
                                    viewEmbed.addFields({
                                      name: sectionResult[0][0].facetName,
                                      value: sectionResult[0][0].facetDesc,
                                    });
                                  }
              ...
              db.query(
                `SELECT * FROM guild_${
                  message.guild.id
                } WHERE LOWER(facetName) = '${nameText.toLowerCase()}'`
              )
                .then((result) => {
                  console.log(result[0]);
                  if (result[0].length > 1) {
                    if (result[0]) {
                      searchEmbed = new Discord.MessageEmbed()
                        .setColor("#99ff00")
                        .setTitle(
                          "There are multiple facets with the name " + nameText
                        )
                        .setDescription(
                          "Please use %wbview [ID] with one of the ID's below"
                        )
                        .setFooter(
                          "You can also use %wbsearch for additional searching options",
                          "https://cdn.discordapp.com/app-icons/592736284816965663/8223af4828cc8d2db1c5fffcca3acfa0.png?size=512"
                        );
