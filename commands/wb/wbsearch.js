            ...
            if (args.length > 1) {
              if (args[0] === "tag") {
                    ...
                    if (result[0] && result[0].length > 0) {
                      ...
                      for (var i = 0; i < result[0].length; i++) {
                        if (result[0][i].facetType != "section") {
                          // if have tags
                          if (
                            result[0][0].facetTags &&
                            result[0][i].facetTags.length > 0
                          ) {
                            // loop tags in entry
                            loop1: for (
                              var j = 0;
                              j < result[0][i].facetTags.split(", ").length;
                              j++
                            ) {
                              let currentTag = result[0][i].facetTags
                                .split(", ")
                                [j].replace(/[^\w\s]/gi, "")
                                .toLowerCase();
                              ...
                              if (
                                currentTag === tagText || // exact match
                                currentTag.substring(
                                  0,
                                  currentTag.length - 1
                                ) === tagText || // match minus trailing "s" in one of the two
                                currentTag ===
                                  tagText.substring(0, tagText.length - 1)
                              ) {
                              ...
                              for (
                                var k = 1;
                                k < currentTag.split(" ").length;
                                k++
                              ) {
                                if (
                                  currentTag.split(" ")[k] === tagText || // exact match
                                  currentTag
                                    .split(" ")
                                    [k].substring(0, currentTag.length - 1) ===
                                    tagText || // match minus trailing "s" in one of the two
                                  currentTag.split(" ")[k] ===
                                    tagText.substring(0, tagText.length - 1)
                                ) {
                              ...
                              for (var k = 1; k < args.length; k++) {
                                console.log(
                                  "arg " + args[k] + " " + currentTag
                                );

                                // all tag match possibilit
                                if (
                                  currentTag === args[k].toLowerCase() ||
                                  currentTag.substring(0, currentTag - 1) ===
                                    args[k].toLowerCase() || // match minus trailing "s" in one of the two
                                  currentTag ===
                                    args[k]
                                      .substring(0, args[k].length - 1)
                                      .toLowerCase() ||
                                  currentTag ===
                                    args[k]
                                      .replace(/[^\w\s]/gi, "")
                                      .toLowerCase() || // match minus trailing "s" in one of the two
                                  currentTag ===
                                    args[k]
                                      .substring(0, args[k].length - 1)
                                      .replace(/[^\w\s]/gi, "")
                      ...
                      if (matchIDs.length > 0) {
                        ...
                        db.query(
                          `SELECT * FROM guild_${message.guild.id} WHERE facetId IN (${IDsText})`
                        )
                          .then((result2) => {
                            if (result2[0] && result2[0].length > 0) {
                              let searchEmbed = new Discord.MessageEmbed()
                                ...
                                .setDescription(
                                  "Includes exact or similar results"
                                )
                                .setFooter(
                              ...
                              for (var i = 0; i < result2[0].length; i++) {
                                let shortDesc = result2[0][i].facetDesc.split(
                                  "."
                                )[0];
                                ...
                                searchEmbed.addFields({
                                  name:
                                    ...
                                    result2[0][i].facetId +
                                    ": " +
                                    result2[0][i].facetName,
                                  value:
                                    "Type: " +
                                    result2[0][i].facetType +
                                    "\nTags: " +
                                    result2[0][i].facetTags +
                                    "\n" +
                                    shortDesc,
                                });
                              }
                      ...
                      for (var i = 0; i < result[0].length; i++) {
                        if (result[0][i].facetType != "section") {
                          ...
                          if (
                            ...
                            result[0][i].facetName.length > 0
                          ) {
                            let name = result[0][i].facetName
                              .replace(/[^\w\s]/gi, "")
                              .toLowerCase();
                            ...
                            for (var k = 1; k < name.split(" ").length; k++) {
                              if (
                                name.split(" ")[k] === nameText || // exact match
                                name
                                  .split(" ")
                                  [k].substring(0, name.length - 1) ===
                                  nameText || // match minus trailing "s" in one of the two
                                name.split(" ")[k] ===
                                  nameText.substring(0, nameText.length - 1)
                              ) {
                                matchIDs.push(result[0][i].facetId);
                            ...
                            for (var k = 1; k < args.length; k++) {
                              ...
                              // all tag match possibilit
                              if (
                                name === args[k].toLowerCase() ||
                                name.substring(0, name - 1) ===
                                  args[k].toLowerCase() || // match minus trailing "s" in one of the two
                                name ===
                                  args[k]
                                    .substring(0, args[k].length - 1)
                              ...
                              for (var l = 1; l < name.split(" ").length; l++) {
                                if (
                                  name.split(" ")[l] ===
                                    args[k].toLowerCase() ||
                                  name.split(" ")[l].substring(0, name - 1) ===
                                    args[k].toLowerCase() || // match minus trailing "s" in one of the two
                                  name.split(" ")[l] ===
                                    args[k]
                                      .substring(0, args[k].length - 1)
                ...
                if (
                  ...
                  db.query(
                    `SELECT * FROM guild_${message.guild.id} WHERE facetType = '${args[1]}'`
                  )
                    .then((result) => {
                      if (result[0] && result[0].length > 0) {
                        let searchEmbed = new Discord.MessageEmbed()
                        ...
                        for (var i = 0; i < result[0].length; i++) {
                          ...
                          searchEmbed.addFields({
                            name:
                              ...
                              result[0][i].facetId +
                              ": " +
                              result[0][i].facetName,
                            value:
                              "Type: " +
                              result[0][i].facetType +
                              "\nTags: " +
                              result[0][i].facetTags +
                              "\n" +
                              shortDesc,
                          });
                        }
                ...
                db.query(
                  `SELECT * FROM guild_${message.guild.id} WHERE facetAuthor = '${args[1]}'`
                )
                  .then((result) => {
                    if (result[0] && result[0].length > 0) {
                      let searchEmbed = new Discord.MessageEmbed()
