...
module.exports = class WBListCommand extends Commando.Command {
    constructor(client) {
        super(client, {
            ...
            details: "%wblist to start listing, %wblist [page] to skip to a certain page",
            argsType: "multiple"
        })
    }
                ...
                if (userResult[0] && userResult[0].length > 0) { // do command
                    db.query(`SELECT * FROM guild_${message.guild.id}`).then(result => {
                        if (result[0] && result[0].length > 0) {
                            ...
                            if (args.length > 0 && Number.isInteger(parseInt(args[0])) && parseInt(args[0]) <= totalPages) {
                                ...
                                let len = (currentPage == totalPages - 1) ? (result[0].length % 5 == 0 ? 5 : result[0].length % 5) : 5;
                                for(var i = 0; i < len; i++) {
                                    let shortDesc = result[0][currentPage*5 + i].facetDesc.split(".")[0];
                                    if (shortDesc.length > 128) shortDesc = shortDesc.substring(0, 127);
                                    listEmbed.addFields(
                                        {name: 'ID #' + result[0][currentPage*5 + i].facetId + ": " + result[0][currentPage*5 + i].facetName, value: "Type: " + result[0][currentPage*5 + i].facetType + "\nTags: " + result[0][currentPage*5 + i].facetTags + "\n" + shortDesc}
                                    )
                                }
                                message.channel.send(listEmbed).then(function (botMessage) {
                                    console.log("HIHI");
                                    botMessage.react("⬅️")
                                    botMessage.react("➡️")
                            ...
                            message.channel.send("No facets exist yet in this server, consider crating one with %wbcreate.");
                        }
                    }).catch(err => console.log(err));
