...
module.exports = class giveCommand extends Commando.Command {
  constructor(client) {
    super(client, {
      ...
      details:
        "%give [user ID or tag] [number of points (int)] inserts into users balance number of points subtracted from your balance.",
      argsType: "multiple",
    });
  }
        ...
        await db
          ...
          .then((result) => {
            if (result[0].length == 0) {
              message.reply(
                "You have not been registered yet! Please start by typing %wbrules and reading them carefully."
              );
            } else {
              if (
              ...
              db.query(`SELECT * FROM users WHERE userId = '${args[0]}'`)
                .then((result2) => {
                  if (result2[0] && result2[0].length > 0) {
                    if (Number.isInteger(parseInt(args[1]))) {
                      if (result[0][0].energy - args[1] >= 0) {
                        ...
                        db.query(
                          `UPDATE users SET energy = ${user1Bal} WHERE userId = '${result[0][0].userId}'`
                        )
                          .then((result3) => {
                            db.query(
                              `UPDATE users SET energy = ${user2Bal} WHERE userId = '${result2[0][0].userId}'`
                            )
                              .then((result4) => {
                        ...
                        message.channel.send(
                          "You do not have enough energy points to make that transaction."
                        );
                      }
                    } else {
