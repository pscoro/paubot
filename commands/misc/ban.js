    ...
    async run(message, args) {
        ...
        if (message.author.id.toString() === owner) {
            ...
            if (args[0] === "remove") {
                ...
                await db.query(`SELECT * FROM bans WHERE userId = '${args[1]}'`).then(result => {
                    if (result[0] && result[0].length > 0) {
                        db.query(`DELETE FROM bans WHERE userId = '${args[1]}'`).then(result => {
                ...
                await db.query(`SELECT * FROM bans WHERE userId = '${args[0]}'`).then(result => {
                    if (result[0] && result[0].length > 0) {
                        ...
                        if(args.length > 2) {
                            ...
                            db.query(`INSERT INTO bans VALUES ('${args[0]}', ${banUntil}, '${banDesc}')`).then(result => {
                                ...
                                db.query(`DELETE FROM users WHERE userId = '${args[0]}'`).then(result => {

                                }).catch(err => console.log(err));
                            }).catch(err => console.log(err));
                        } else {
                            db.query(`INSERT INTO bans (userId, banUntil) VALUES ('${args[0]}', ${banUntil})`).then(result => {
